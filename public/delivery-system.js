...
// Past Job Mode - เหลือแค่รอบเย็นหลัง 4 โมงเท่านั้น
const TimeSlots = [
    {
        id: 'EVENING',
        name: '16:00 - 20:00',
        nameTh: 'รอบเย็น 16:00 - 20:00',
        nameEn: 'Evening 4PM - 8PM',
        startTime: '16:00',
        endTime: '20:00',
        maxOrders: 15, // รับได้มากขึ้นเพราะเป็นรอบเดียว
        isActive: true
    }
];
...
const DeliverySystem = {
    ...
    // Update time slots UI based on selected date - Past Job Mode: Evening only
    updateTimeSlotsUI(dateStr) {
        const slots = DeliveryQuota.getAvailableSlots(dateStr);

        // Update evening slot only (Past Job Mode)
        ...
        const slotFull = slots.length > 0 && slots.every(s => s.remaining <= 0);
        const quotaWarning = document.getElementById('quotaWarning');
        if (quotaWarning) {
            quotaWarning.style.display = slotFull ? 'flex' : 'none';
        }
    },
    ...
    // Get selected time slot - Past Job Mode: Always EVENING
    getSelectedTimeSlot() {
        const selected = document.querySelector('input[name="timeSlot"]:checked');
        return selected?.value || 'EVENING'; // Default to EVENING for Past Job Mode
    },
